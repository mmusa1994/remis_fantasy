...
import { getSeasonLabel } from "./fpl-season";
...
function getMultilingualErrorMessage(query: string): string {
  ...
  if (isBosnia) {
    return `Pitaj me nešto za Fantasy Premier League sezonu ${season}, ništa drugo me ne zanima. Primjer: "Ko je najbolji kapiten za GW3?" ili "Koji su najbolji diferensijali?"`;
  } else if (isSerbian) {
    return `Питај ме нешто за Fantasy Premier League сезону ${season}, ништа друго ме не занима. Пример: "Ко је најбољи капитен за GW3?" или "Који су најбољи диференцијали?"`;
  } else {
    return `Ask me something about Fantasy Premier League ${season} season, nothing else interests me. I'll respond in your language. Examples: "Who should I captain for GW3?" or "What are the best differentials this week?"`;
  }
}
