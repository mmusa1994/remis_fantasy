...
const TransferTrendsWidget = React.memo<TransferTrendsWidgetProps>(function TransferTrendsWidget({
  currentGameweek = 4,
  ...
  const [activeTab, setActiveTab] = useState<'current' | 'players_in' | 'players_out' | 'future'>('current');
   ...
   */
  const isCacheValid = useCallback((cache: WidgetCache | null): boolean => {
  ...
  const fetchTransferTrends = useCallback(async (forceRefresh: boolean = false, gameweek: number = 4) => {
    ...
    if (cacheRef.current && !forceRefresh) {
      ...
      if (timeSinceLastFetch < MIN_REFRESH_INTERVAL) {
        console.log(`[TransferTrendsWidget] Rate limited: ${Math.round((MIN_REFRESH_INTERVAL - timeSinceLastFetch) / 1000)}s remaining`);
        setLoading(false);
        return;
      }
    }
    ...
    <div className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-lg p-4 shadow-lg border border-gray-200 dark:border-gray-700`}>
      ...
      <div className="space-y-3">
        <AnimatePresence mode="wait">
              ...
              {data.current_transfers.map((transfer, index) => (
                ...
                >
                  <div className="flex items-center space-x-2 text-sm">
                    <span className="font-medium truncate">{transfer.player_out.name}</span>
                    <ArrowRightLeft className="w-3 h-3 text-gray-400" />
                    <span className="font-medium truncate">{transfer.player_in.name}</span>
                  </div>
          ...
          {/* Future Transfers */}
          {activeTab === 'future' && showFutureWeeks && data.future_transfers && (
            <motion.div
              key="future"
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              {Object.entries(data.future_transfers).slice(0, 3).map(([week, transfers]) => (
                <div key={week} className="space-y-2">
                  <div className="text-xs font-medium text-gray-600 dark:text-gray-400">
                    Week {week}
                  </div>
                  {(transfers as any).planned_transfers?.slice(0, 3).map((transfer: any, index: number) => (
                    <div
                      key={index}
                      className="flex items-center justify-between p-2 rounded-lg bg-blue-50 dark:bg-blue-900/20 text-sm"
                    >
                      <div className="flex items-center space-x-2">
                        <span className="truncate">{transfer.player_out_name}</span>
                        <ArrowRightLeft className="w-3 h-3 text-gray-400" />
                        <span className="truncate">{transfer.player_in_name}</span>
                      </div>
