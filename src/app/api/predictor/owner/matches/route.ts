...
import { requireTournamentOwner, jsonError, rescoreMatch } from "@/lib/predictor";

async function tournamentForMatch(matchId: string): Promise<string | null> {
...
const baseFields = (tournament_id: string, m: any) => ({
  tournament_id,
  stage: m.stage ?? "group",
...
export async function POST(req: NextRequest) {
  ...
  if (Array.isArray(body.matches)) {
    const rows = body.matches
      ...
      .map((m: any, idx: number) => ({
        ...baseFields(body.tournament_id, m),
  ...
  if (!body.home_team || !body.away_team)
    return jsonError("home_team and away_team are required");
  const { data, error } = await supabaseServer
    .from("predictor_matches")
    .insert(baseFields(body.tournament_id, body))
    .select()
    .single();
  if (error) return jsonError(error.message, 500);
  return NextResponse.json(data);
...
export async function PUT(req: NextRequest) {
  ...
  const tid = await tournamentForMatch(body.id);
  if (!tid) return jsonError("Match not found", 404);
...
export async function DELETE(req: NextRequest) {
  ...
  const tid = await tournamentForMatch(id);
  if (!tid) return jsonError("Match not found", 404);
  const own = await requireTournamentOwner(tid);
  if (!own.ok) return own.response;
  const { error } = await supabaseServer.from("predictor_matches").delete().eq("id", id);
  if (error) return jsonError(error.message, 500);
