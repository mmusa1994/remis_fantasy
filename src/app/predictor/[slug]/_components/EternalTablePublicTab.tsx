...
export default function EternalTablePublicTab({
  ...
  if (error || (columns.length === 0 && entries.length === 0)) {
    return null;
  }

  // Tiny helper — top-3 medal accent color for the rank number.
  const rankColor = (idx: number, isDark: boolean) => {
    if (idx === 0) return "text-amber-500"; // gold
    if (idx === 1) return isDark ? "text-slate-300" : "text-slate-500"; // silver
    if (idx === 2) return isDark ? "text-orange-300" : "text-orange-600"; // bronze
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Trophy className="h-4 w-4 sm:h-5 sm:w-5 text-amber-500" strokeWidth={2.25} />
        </div>
          ...
          return (
            ...
            >
              {isTop && (
                <span
                  aria-hidden
                  className="absolute left-0 top-3 bottom-3 w-[3px] rounded-r-full bg-amber-500"
                />
              )}
          ...
          <tbody>
            {entries.map((e, i) => {
              ...
              return (
                ...
                >
                  {/* Rank — fixed width, baseline stable across all rows */}
                  <td
                    className={`relative pl-5 pr-2 py-4 w-14 text-center align-middle`}
                  >
                    {isTop ? (
                      <span
                        aria-hidden
                        className="absolute left-0 top-1/2 -translate-y-1/2 h-7 w-[3px] rounded-r-full bg-amber-500"
                      />
                    ) : null}
                    <span
                  ...
                  {/* Total — subtle pill */}
                  <td
                    className={`pl-3 pr-5 py-4 text-center align-middle`}
                  >
                    <span
